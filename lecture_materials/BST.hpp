...
*
* Шаблон на клас, представящ двоично наредено дърво.
* Илюстрира базова имплементация на ддт.
*
* Към шаблонния тип налагаме изискване да притежава:
* подразбиращ се конструктор, копиращ конструктор,
* оператор за присвояване, оператор за еквивалентност
* оператор за сравнение (<) и възможност за изход (<<)
*
* Петър Армянов, 2020
***************************************************************/
...
public:
    ...
    void insert(const DataType& key) { insert(root, key); }
...
private:
    ...
    bool findIter(const DataType& key) const
    {
        node* current = root;
        while (current) {
    ...
    void insertIter(const DataType& key) const
    {
...
/*
                ...
                if (!root->right->left) {
                    ...
                    node* par = root->right;
                    mR = par->left;
                    while (mR->left){ par = mR; mR = par->left;}
                    par->left = mR->right;
                }
*/
