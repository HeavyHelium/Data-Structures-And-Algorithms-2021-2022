...
/// added partial order operators because they are used for determining priority at times
...
private:
	struct resource_state
	{
		...
		int estimate() const { return in_stock + arriving; }
	};
	...
	list<MyClient> clients;//< list of all clients
	list<client_remaining> clients_in_order;//< stores clients who are waiting in order of arrival
	list_with_sorted_insertion<client_waiting> clients_waiting;//< stores clients who are waiting in order of max departure minute

	/// @brief sends worker for resource at a given minute
	/// @param T ResourceType
	/// @param minute int
	void sendWorker(ResourceType T, int minute);
	/// @brief makes a delivery of arriving resources
	void delivery(int minute);
	/// @brief client departure at a given minute
	void clientDeparture(const MyClient& client, int minute);
	/// @brief if amount is <= amount in stock, then amount units are taken, 
	/// else the whole resorce in stock is taken 
	...
	/// @brief determines whether or not a client can be serviced
	...
	/// not to be confused with advanceTo since it does not add clients, compared to the former
	/// @param minute int
	void actionBefore(int minute);
	/// @brief at a given minute does the following:
	/// welcomes clients -- sends workers for banana, workers for schweppes,
	/// then delivery of resources occurs,
	/// then servicing of clients
	/// @param minute int 
	void actionAt(int minute);
	/// @brief all clients whose max depart minute <= minute depart in order of arrival 
	void releaseClientsByOrderBefore(int minute);
	/// @brief all clients whose max depart minute <= minute depart in order of max_departure minute 
	void releaseClientsByNecessityBefore(int minute);
	list<MyClient>::iterator iter = nullptr;//< iterator to first not yet welcomed client
};
...
//"реда на операциите в една минута
//- идват клиенти
//- пращат се работници
//- връщат се работници
//- клиенти си тръгват"
void MyStore::actionAt(int minute)
{
	...
	for (typename list<MyClient>::iterator& elem : to_be_welcomed)
	{
		int banana_order = 0;
		...
		//"в условието не е указано как се избира ако има повече от 1 работник,
		//но сме имали предвид винаги да се избира по този начин и само примера да е с 1,"
		...
		// at a given minute workers for banana are sent first
		while (sent_for_banana) {
			sendWorker(ResourceType::banana, minute); 
			--sent_for_banana;
		}
		// then those for schweppes
