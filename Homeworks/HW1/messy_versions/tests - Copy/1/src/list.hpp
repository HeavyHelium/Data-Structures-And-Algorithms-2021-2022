...
template<typename T>
typename list<T>::iterator list<T>::insert(iterator position, const T& elem)
{
    if (position == end()) { return push_back(elem); }
    if (position == this->begin()) { return push_front(elem); }
...
void list<T>::erase(iterator position)
{
    ...
    if (position == iterator(last)) { pop_back(); return; }
