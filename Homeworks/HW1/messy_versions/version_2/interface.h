...
struct MyStore : Store
{
	...
	{
//std::cout << "here " << clients_waiting.size() << std::endl;
//std::cout << "here " << arriving_goods.size() << std::endl;
	    while(!clients_waiting.empty()
             && (clients_waiting.top().max_departure_time() <= minute)
             && (arriving_goods.empty() || (!arriving_goods.empty()
                && arriving_goods.front().time > clients_waiting.top().max_departure_time())))
            pop_client();

        while(!arriving_goods.empty() && arriving_goods.front().time <= minute)
        {
            // see if there are any clients
            int current_time = arriving_goods.front().time;
            //while(!arriving_goods.empty() && arriving_goods.front() == current_time) pop_resource();
           // for(auto iter = arriving_goods.begin(); iter != arriving_goods.end(); ++iter)
        }
	...
	void pop_client()
	{
	    assert(!clients_waiting.empty()
            && "Cannot remove clients when there are no such waiting");
	...
	void pop_resource()
	{
	    assert(!arriving_goods.empty()
            && "cannot pop a resource when there is no such");
	...
	void addClient(const Client& c)
	{
        ...
        else
        {
            ...
            if(c.maxWaitTime >= RESTOCK_TIME) banana.to_be_taken += c.banana;
            if(c.maxWaitTime >= RESTOCK_TIME) schweppes.to_be_taken += c.schweppes;
        }
